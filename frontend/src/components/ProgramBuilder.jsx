...
// Each slot in the calendar: { week_number, day_of_week, routine_id, routine_name, is_rest_day }
function buildEmptyWeeks(count) {
  const weeks = [];
  for (let w = 1; w <= count; w++) {
    weeks.push({ week: w, slots: Array(7).fill(null) });
  }
  return weeks;
}
...
export default function ProgramBuilder({ existingProgram, onSaved, onClose }) {
  ...
  const [dragging, setDragging] = useState(null); // { routine_id, routine_name } or { from: [wi, dow] }
  ...
  // Resize weeks when duration changes
  useEffect(() => {
    setWeeks(prev => {
      if (prev.length === durationWeeks) return prev;
      if (durationWeeks > prev.length) {
        const extra = buildEmptyWeeks(durationWeeks - prev.length).map((w, i) => ({ ...w, week: prev.length + i + 1 }));
        return [...prev, ...extra];
      }
  ...
  // Drag from routine list
  const onDragStartRoutine = (routine) => {
    setDragging({ routine_id: routine.id, routine_name: routine.name });
  };

  // Drag from existing slot
  const onDragStartSlot = (wi, dow) => {
    setDragging({ from: [wi, dow], ...weeks[wi].slots[dow] });
  };

  const onDropSlot = (wi, dow, type) => {
    if (!dragging) return;
    setWeeks(prev => {
      const next = prev.map(w => ({ ...w, slots: [...w.slots] }));
      // Clear source slot if dragging from calendar
      if (dragging.from) {
        const [fwi, fdow] = dragging.from;
        next[fwi].slots[fdow] = null;
      }
      // Set target slot
      next[wi].slots[dow] = type === 'rest'
  ...
  return (
    <div className="pb-container">
      ...
      <div className="pb-section">
        ...
        <div className="pb-duration-row">
          {DURATION_PRESETS.map(n => (
            <button
              key={n}
              className={`pb-duration-btn ${durationWeeks === n && !customDuration ? 'active' : ''}`}
      ...
      <div className="pb-section">
        ...
        <p className="pb-hint">When a workout is completed late, how should the rest of the program adjust?</p>
        <div className="pb-shift-row">
          {[
            ...
            <button
              key={opt.value}
              className={`pb-shift-btn ${scheduleShift === opt.value ? 'active' : ''}`}
      ...
      <div className="pb-section">
        ...
        <p className="pb-hint">Drag routines from the list onto calendar days. Right-click a slot to set it as a rest day or clear it.</p>
        ...
        <div className="pb-builder-layout">
          ...
          <div className="pb-routine-palette">
            ...
            {routines.map(r => (
              <div
                key={r.id}
                className="pb-routine-chip"
                draggable
                onDragStart={() => onDragStartRoutine(r)}
              >
                {r.name}
              </div>
            ))}
            <div
              className="pb-routine-chip pb-rest-chip"
              draggable
          ...
          <div className="pb-calendar">
            ...
            <div className="pb-cal-row pb-cal-header-row">
              ...
              {DAY_NAMES.map(d => (
                <div key={d} className="pb-cal-cell pb-cal-header">{d}</div>
              ))}
            </div>
              ...
              <div key={wi} className="pb-cal-row">
                ...
                {wk.slots.map((slot, dow) => (
                  <div
                    ...
                    onContextMenu={e => {
                      e.preventDefault();
                      if (slot) clearSlot(wi, dow);
                      else setRestDay(wi, dow);
                    }}
                  >
                    {slot && !slot.is_rest_day && (
                      <div
                        className="pb-slot-content"
                        draggable
                        onDragStart={() => onDragStartSlot(wi, dow)}
                      >
